...
class ImportServer {
  ...
  async getRandomProfileImageUrl() {
    ...
    const profiles = await got.get('https://randomuser.me/api').json();
    let image_url = profiles['results'][0]['picture']['thumbnail'];
    ...
    let request = {
      form: {
        ...
        Participants: JSON.stringify([{
          ...
          image_url: await this.getRandomProfileImageUrl()
        }, {
          channel: 2,
          type: 'agent',
          full_name: agent_names[Math.floor(Math.random() * agent_names.length)],
          image_url: await this.getRandomProfileImageUrl()
        }])
      },
